...
class RectTextObject(RectObject):
    def __init__(self, color, rect, square, border, radius, text=" ", text_color=(0, 0, 0), text_size=1.0):
        ...
        self.font = pygame.font.Font('fonts/Quicksand-Regular_afda0c4733e67d13c4b46e7985d6a9ce.ttf',
                                     int(self.text_size * min(self.width / (len(self.text) / 2), self.height) * 3 / 5))
...
class RectTextButton(RectTextObject):
    def __init__(self, color, rect, square, border, radius, action, text=" ", text_color=(0, 0, 0), text_size=1.0):

        super().__init__(color, rect, square, border, radius, text, text_color, text_size)
    ...
    def draw(self, surface, selected):
        if selected:
            pygame.draw.rect(surface, (self.color[0], self.color[1], self.color[2], 160),
                             (self.x, self.y, self.width, self.height), self.border, self.radius)
        else:
...
class PromotionButton(ImageButton):
    def __init__(self, color, rect, square, border, radius, action, piece, piece_images):
        self.piece_color = "w"
        self.piece = piece
        super().__init__(color, rect, square, border, radius, action, "", ' ', (0, 0, 0, 0))
...
class Board(RectObject):
    def __init__(self, colors, rect):
        ...
        super().__init__((0, 0, 0, 0), rect, True, 0, 0)
    ...
    def scale(self, screen_size):
        ...
        self.width = self.sq_size * 8
        self.height = self.sq_size * 8
    ...
    def draw_arrows(self, surface, coordinates):

        new_surface = pygame.Surface((self.width, self.height), pygame.SRCALPHA).convert_alpha()

        for coordinate in coordinates:

            origin_row, origin_col, target_row, target_col = coordinate

            thickness = self.sq_size // 6
            origin_y = origin_row * self.sq_size + self.sq_size // 2
            origin_x = origin_col * self.sq_size + self.sq_size // 2 - thickness // 4

            target_y = target_row * self.sq_size + self.sq_size // 2
            target_x = target_col * self.sq_size + self.sq_size // 2 - thickness // 4

            pygame.draw.line(new_surface, ARROW_COLORS[0], (origin_x, origin_y), (target_x, target_y), thickness)

        surface.blit(new_surface, (self.x, self.y))


class Piece(pygame.sprite.Sprite, RectObject):
    def __init__(self, rect, board_starting_square, row, col, side, piece, piece_images):
        RectObject.__init__(self, (0, 0, 0, 0), rect, True, 0, 0)
        ...
        self.board_starting_square = board_starting_square

        self.starting_square_ratio = (DEFAULT_SCREEN_SIZE[0] / self.board_starting_square[0],
                                      DEFAULT_SCREEN_SIZE[1] / self.board_starting_square[1],
                                      DEFAULT_SCREEN_SIZE[0] / (self.width * 8),
                                      DEFAULT_SCREEN_SIZE[1] / (self.height * 8))

        self.new_starting_square_location = (0, 0)

    def scale(self, screen_size):
        RectObject.scale(self, screen_size)
        ...
        new_starting_square_size[0] = new_starting_square_size[0] // 8 * 8
        new_starting_square_size[1] = new_starting_square_size[1] // 8 * 8
